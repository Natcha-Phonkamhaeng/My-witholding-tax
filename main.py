...
class App_Inquiry:
			...
			for i in self.cash_path:
				...
				for f in glob.glob(i):
					try:
						...
						self.df_cash = self.df_cash.append(df_concat, ignore_index=True)
		...
		def browse_inq(self):
			...
			for i in self.inq_path:
				...
				for f in glob.glob(i):
					read_inq = pd.read_excel(f)
					self.df_inq = self.df_inq.append(read_inq, ignore_index=True)
			try:
		...
		def draw(self):
			...
			self.journal_btn.grid(row=0, column=0, padx=80, pady=20)
			self.clear_btn.grid(row=0, column=4, padx=50)
			self.inq_btn.grid(row=0, column=1)
			self.merge_btn.grid(row=1, column=4, padx= 50)
			
			self.listbox_journal.grid(row=1, column=0, padx=80)
			self.listbox_inq.grid(row=1, column=1)
...
class App_WHT:
	...
	def __init__(self, master):
		...
		self.tree_view = ttk.Treeview(self.frame, height=18, selectmode='extended', yscrollcommand=self.y_scrollbar.set)
	...
	def draw(self):
		...
		df_rows = app_inquiry.df_result.filter(items=filt).to_numpy().tolist()		
